-- ***************************************************************
-- This script creates and populates in MySQL
-- all sample tables (and the one view)
-- of the DATABASE FOUNDATIONS Oracle Academy course
-- ***************************************************************

-- Als er reeds een database met de naam "DB_Foundations" bestaat
...
-- Weet je zeker dat je de bestaande "DB_Foundations"-database wilt wissen
...
DROP DATABASE db_foundations;

CREATE DATABASE db_foundations;
USE db_foundations;

-- Drop all tables if they exist

-- The DROP statements are legacy code from the APEX version of this script.
-- They are no longer usefull since, in MySQL, we DROP en CREATE the entire DATABASE.
-- (In APEX there is only ONE database for every user,
-- there is no way to DROP, CREATE or select another DATABASE.)

-- SET foreign_key_checks = 0;  -- ignore foreign key constraints
                                -- this solved troubles with FOREIGN KEY CONSTRAINTS
                                -- pointing to eachother, preventing succesful DROP TABLES
...
SET foreign_key_checks = 1;  -- enforce foreign key constraints
                             -- make sure it's enabled, in case it was ever disabled before
...
CREATE TABLE  wf_countries
   ( COUNTRY_ID  DECIMAL(4,0), 
	 ...
	 CAPITOL  VARCHAR(50), 
	 AREA  DECIMAL(15,0), 
	 COASTLINE  DECIMAL(8,0), 
	 LOWEST_ELEVATION  DECIMAL(6,0), 
	 LOWEST_ELEV_NAME  VARCHAR(70), 
	 HIGHEST_ELEVATION  DECIMAL(6,0), 
...
CREATE TABLE   d_packages 
   ( CODE  DECIMAL(5,0),
     LOW_RANGE  DECIMAL(6,0) NOT NULL,
     HIGH_RANGE  DECIMAL(6,0) NOT NULL,
...
CREATE TABLE   d_events 
   ( ID  DECIMAL(5,0),
     ...
     VENUE_ID  DECIMAL(5,0) NOT NULL,
     PACKAGE_CODE  DECIMAL(5,0) NOT NULL,
     THEME_CODE  DECIMAL(10,0) NOT NULL,
     CLIENT_NUMBER  DECIMAL(5,0) NOT NULL,
...
CREATE TABLE   d_songs 
   ( ID  DECIMAL(5,0),
     ...
     DURATION  VARCHAR(20),
     ARTIST  VARCHAR(20),
     TYPE_CODE  DECIMAL(5,0) NOT NULL,
...
--  	REFERENCES   employees  ( EMPLOYEE_ID ) DISABLE; (= Oracle code)
--  In MySQL we can't DISABLE/ENABLE a foreign key.
--  So, we choose not to create this foreign key here,
--  but to create it later on (line 3663).
...
--  ALTER TABLE DEPARTMENTS  ENABLE  CONSTRAINT  DEPT_MGR_FK ; (= Oracle code)
--  In MySQL we can't DISABLE/ENABLE a foreign key.
--  So, we have chosen not to create this foreign key in line 3471,
--  but we create it here.
ALTER TABLE   departments  ADD CONSTRAINT  DEPT_MGR_FK  FOREIGN KEY ( MANAGER_ID )
	REFERENCES   employees  ( EMPLOYEE_ID ) ;
...
/* SEQUENCES ARE NOT SUPPORTED IN MYSQL; MYSQL USES THE AUTO_INCREMENT COLUMN ATTRIBUTE.

-- In the original Oracle version of this script, the three sequences below are created, but not used.
